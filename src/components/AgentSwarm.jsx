    ...
    <div className="w-full fluid-container py-6 sm:py-10 animate-fade-in space-y-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <motion.button
          ...
          disabled={isRunning}
          className="px-6 py-3 rounded-xl bg-[#007AFF] hover:bg-[#5EFCC2] text-white font-bold text-xs sm:text-sm shadow-md shadow-[#007AFF]/20 flex items-center gap-2"
        >
          {isRunning ? (
            <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {simpleAgents.map((agent) => {
          ...
          return (
            <div
              key={agent.id}
              className="p-5 rounded-2xl bg-white dark:bg-white dark:bg-[#1D1D1F] border border-slate-200/80 dark:border-[#AAAAAA]/30 dark:border-white/[0.08] hover:border-[#007AFF] transition-all shadow-sm flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-[#F5F5F7] dark:bg-white/[0.04] border border-slate-200 dark:border-[#AAAAAA]/30 dark:border-white/[0.08] flex items-center justify-center">
                    <Icon className={`w-5 h-5 text-[#007AFF]`} />
                  </div>
                  <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-[#007AFF]/10 text-[#007AFF] border border-[#007AFF]/30 flex items-center gap-1 font-mono">
